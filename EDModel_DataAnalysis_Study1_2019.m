% Create a varaible named 'Data' and copy the 'EDModel_Study1_2019' sheet from the excel
% file into this variable (not include the first line)
[M, N] = size(Data);
...
normMu_X = [];      normMu_Y = [];
corr = [];
time = [];  allDataNum = [];	errorNum = [];
combNum = 24;    %Number of A * D combination

%Analysis
for i = 1:round(M/20)
    list = (i-1)*20+1:(i)*20;
    L_X = Data(list,2);   L_Y = Data(list,3);  
    L_Time = Data(list,4);  L_Error = Data(list, 7);
    keep = Data(list,N+1);
